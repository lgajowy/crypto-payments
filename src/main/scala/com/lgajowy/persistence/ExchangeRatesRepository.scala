...
class ExchangeRatesRepository {
  ...
  def getCoinExchangeRate(fiatCurrency: FiatCurrency, coinCurrency: CoinCurrency): Option[ExchangeRate] = {
    coinCurrency match {
      case CoinCurrency("BTC") => MarketData.exchangeRatesOfBTC.get(fiatCurrency.value).map(ExchangeRate)
      case _                   => None
    }
  }
...
object ExchangeRatesRepository {
  def apply(): ExchangeRatesRepository = new ExchangeRatesRepository()
}
