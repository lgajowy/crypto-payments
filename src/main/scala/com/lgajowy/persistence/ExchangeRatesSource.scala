...
object ExchangeRatesSource {
  ...
  def make(): ExchangeRatesSource = new ExchangeRatesSource {
    override def getEurExchangeRate(fiatCurrency: FiatCurrency): Option[ExchangeRate] = {
      MarketData.exchangeRatesToEUR.get(fiatCurrency.value).map(ExchangeRate)
    }

    override def getCoinExchangeRate(fiatCurrency: FiatCurrency, coinCurrency: CoinCurrency): Option[ExchangeRate] = {
      coinCurrency match {
